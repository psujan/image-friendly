...
const compressService = async (req, res) => {
    try {
        await ensureTempDir();
        const imagePath = path.join(process.cwd(), 'public', 'uploads', req.body.imageName);
...
const compressImage = async (buffer, options = {}) => {
    ...
    if (brightness) transformer = transformer.modulate({brightness: parseFloat(brightness)});
    if (contrast) transformer = transformer.modulate({saturation: parseFloat(contrast)});
    if (saturation) transformer = transformer.modulate({saturation: parseFloat(saturation)});
    ...
    const finalProgressive = preset ? compressionPresets[preset].progressive : progressive;
